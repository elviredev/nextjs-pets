...
export default function Chat() {
    ...
    // Connexion Serveur de tchat - Pusher
    useEffect(() => {
    ...
    // surveille les changements de messageLog pour scroll barre verticale
    useEffect(() => {
        if (messageLog.length) {
            // régler le scroll tout en bas de la hauteur maximale
            chatLogElement.current.scrollTop = chatLogElement.current.scrollHeight
            // si tchat fermé, incrémenter le badge de msg non lus
            if (!isChatOpen) {
    ...
    // Ouvrir la fenêtre de tchat et réinitialiser le compteur de msg non lus
    function openChatClick() {
        setIsChatOpen(true)
        setUnreadCount(0)
        // Focus sur champ input
        setTimeout(() => {
            chatField.current.focus()
        }, 350)
    }

    // Fermer la fenêtre du tchat
    ...
    function handleChatSubmit(e) {
        ...
        // envoyer le message à notre copie locale de l'état
        setMessageLog(prev => [...prev, {selfMessage: true, message: userMessage}])
        // vider le champ input à chaque soulission du form
        setUserMessage("")
    }

    // Gérer le champ input
        ...
        <>
            ...
            <div className={isChatOpen ? "chat-container chat-container--visible" : "chat-container"}>
                <div className="chat-title-bar">
                    ...
                    <svg
                        onClick={closeChatClick} xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                ...
                <div ref={chatLogElement} className="chat-log">
                    {messageLog.map((item, index) => {
                        return (
                            <div
                                key={index}
                                className={item.selfMessage ? "chat-message chat-message--self" : "chat-message"}
                            >
